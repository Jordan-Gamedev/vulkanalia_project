...
impl VulkanRenderer {
    pub fn new() -> Result<Self> {
        unsafe {
            ...
            // Get device queues
            let device_queue_handle = get_device_graphics_present_queues(
                device.clone(),
                &instance,
                physical_device,
                &surface,
            ...
            let swapchain_handle =
                create_swapchain(&device_context, &window_handle.window, surface)?;

            // Create screen color texture
            let color_texture =
                create_color_texture(&device_context, &swapchain_handle, msaa_samples)?;

            // Create screen depth texture
            let depth_texture =
                create_depth_texture(&device_context, &swapchain_handle, msaa_samples)?;
            ...
            let base_render_pass =
                create_base_render_pass(&device_context, &swapchain_handle, msaa_samples)?;
            ...
            let (compute_pipeline, compute_pipeline_layout) =
                create_compute_pipeline(descriptor_set_layout, &device_context)?;
            ...
            // Create indirect draw call buffers
            let indirect_draw_buffers = create_indirect_draw_buffers(&device_context, command_pool);
            ...
            let command_buffers =
                create_command_buffers(device.clone(), command_pool, framebuffers.len())?;
    ...
    pub fn destroy(&mut self) {
        ...
        unsafe {
            ...
            device.destroy_pipeline(self.render_pipeline_handle.compute_pipeline, None);
            device
                .destroy_pipeline_layout(self.render_pipeline_handle.compute_pipeline_layout, None);
            ...
            if self.command_handle.command_pool != vk::CommandPool::null()
                && !self.command_handle.command_buffers.is_empty()
            {
                device.free_command_buffers(
                    self.command_handle.command_pool,
                    &self.command_handle.command_buffers,
                );
            }
            if self.command_handle.command_pool != vk::CommandPool::null() {
    ...
    pub fn add_instance(
        ...
        // Manually reallocate instance buffer
        if instance_buffer.element_count > instance_buffer.element_capacity {
            // Get items from current buffer
            let mut instances: Vec<PerInstanceData> = unsafe {
                std::slice::from_raw_parts(
                    instance_buffer.mapped.cast(),
                    instance_buffer.element_capacity as usize,
                )
                .to_vec()
            };

            // Add empty instances to the end of the buffer
            instances.extend(vec![
                PerInstanceData::default();
                instance_buffer.alloc_dealloc_threshold as usize
            ]);
            instance_buffer.recreate(
                &self.device_context,
                self.command_handle.command_pool,
                instances,
            );
            instance_buffer.available_indices.clear();
        }

        // Manually reallocate model matrix buffer
        if model_matrix_buffer.element_count > model_matrix_buffer.element_capacity {
            // Get items from current buffer
            let mut model_matrices: Vec<QuantizedModelMatrix> = unsafe {
                std::slice::from_raw_parts(
                    model_matrix_buffer.mapped.cast(),
                    model_matrix_buffer.element_capacity as usize,
                )
                .to_vec()
            };

            // Add empty model matrices to the end of the buffer
            model_matrices.extend(vec![
                QuantizedModelMatrix::default();
                model_matrix_buffer.alloc_dealloc_threshold as usize
            ]);
            model_matrix_buffer.recreate(
                &self.device_context,
                self.command_handle.command_pool,
                model_matrices,
            );
        ...
        for i in 0..instance_buffer.element_count as usize {
            ...
            let buffer_model_matrix = unsafe { model_matrix_buffer.mapped.add(i).read() };

            if buffer_instance.mesh_metadata_index == moving_instance.mesh_metadata_index {
                in_proper_instance_bucket = true;
            } else if in_proper_instance_bucket {
                unsafe {
    ...
    pub fn load_mesh(&mut self, mesh_asset_id: AssetId) -> Result<u16> {
        ...
        let total_vertex_byte_count =
            vertex_byte_count0 + vertex_byte_count1 + vertex_byte_count2 + vertex_byte_count3;
    ...
    pub fn load_texture(
        ...
        let (staging_buffer, staging_buffer_memory, staging_buffer_mapped) =
            Buffer::<*const c_void>::create_buffer(
                &device_context,
                total_size,
        ...
        let sampler = if let Some(usage) = self.texture_handle.samplers.get_mut(&sampler_contents) {
            ...
            self.texture_handle
                .samplers
                .insert(sampler_contents, sampler_usage);
    ...
    pub fn unload_texture(
        ...
        let (unloading_texture, fully_unloaded) = if let Some(texture) = self
            .texture_handle
            .loaded_textures
            .get_mut(&texture_asset_id)
        {
        ...
        if fully_unloaded {
            ...
            unsafe {
                ...
                self.device_context
                    .device
                    .destroy_image(unloading_texture.texture.image, None);
                self.device_context
                    .device
                    .free_memory(unloading_texture.texture.image_memory, None);

                // Unload sampler if no more textures use this sampler
                let sampler = self.texture_handle.samplers[&sampler_contents]
                    .sampler
                    .clone();
    ...
    fn recreate_swapchain(&mut self) -> Result<()> {
        unsafe {
            ...
            self.render_pipeline_handle.descriptor_handle.descriptor_sets = create_descriptor_sets(
                &self.device_context,
                &self.model_handle,
                &self.command_handle.indirect_draw_buffers,
                &self.command_handle.instance_buffers,
    ...
    pub fn render(&mut self) -> Result<()> {
        unsafe {
            ...
            let signal_semaphores =
                &[self.command_handle.sync_handle.render_finished_semaphores[image_index]];
            ...
            device.queue_submit(
                self.device_context.device_queue_handle.graphics_queue,
                &[submit_info],
                in_flight_fence,
            )?;
            ...
            let result = device.queue_present_khr(
                self.device_context.device_queue_handle.present_queue,
                &present_info,
            );
...
unsafe fn create_logical_device(
    ...
    let (graphics_index, present_index, compute_index) =
...
fn set_default_msaa(device_context: &DeviceContext) -> vulkanalia::vk::SampleCountFlags {
    let max_msaa = get_max_msaa_samples(
        device_context.clone().instance,
        device_context.physical_device,
    );
...
unsafe fn create_swapchain(
    ...
    let swapchain_capabilities = device_context
        .instance
        .get_physical_device_surface_capabilities_khr(device_context.physical_device, surface)?;

    let surface_format = get_swapchain_surface_format(
        device_context.clone().instance,
        device_context.physical_device,
        surface,
    );
    let present_mode = get_swapchain_present_mode(
        device_context.clone().instance,
        device_context.physical_device,
        surface,
    );
    ...
    let image_sharing_mode = if device_context
        .device_queue_handle
...
unsafe fn create_descriptor_set_layout(device: Device) -> Result<vk::DescriptorSetLayout> {
    ...
    let model_matrix_binding = vk::DescriptorSetLayoutBinding::builder()
        .binding(3)
        .descriptor_type(vk::DescriptorType::STORAGE_BUFFER)
        .descriptor_count(1)
        .stage_flags(vk::ShaderStageFlags::VERTEX);
...
unsafe fn create_descriptor_pool(device: Device) -> Result<vk::DescriptorPool> {
    ...
    let model_matrix_size = vk::DescriptorPoolSize::builder()
...
unsafe fn create_descriptor_sets(
    ...
    for i in 0..MAX_FRAMES_IN_FLIGHT as usize {
        ...
        let model_matrix_write = vk::WriteDescriptorSet::builder()
            .dst_set(descriptor_sets[i])
            .dst_binding(3)
            .dst_array_element(0)
            .descriptor_type(vk::DescriptorType::STORAGE_BUFFER)
        ...
        let visbuffer_write = vk::WriteDescriptorSet::builder()
            .dst_set(descriptor_sets[i])
            .dst_binding(7)
            .dst_array_element(0)
            .descriptor_type(vk::DescriptorType::STORAGE_BUFFER)
            .buffer_info(&visbuffer_info);
...
unsafe fn create_compute_pipeline(
    ...
    let shader = include_bytes!("../../assets/shaders/culling.spv");
...
pub fn update_bindless_texture(
    ...
    for descriptor_set in descriptor_sets {
        ...
        unsafe {
            device_context
                .device
                .update_descriptor_sets(&[write_set], &[] as &[vk::CopyDescriptorSet]);
        }
    }
    Ok(())
}
