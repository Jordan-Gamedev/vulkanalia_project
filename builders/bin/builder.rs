...
fn create_resources() -> Result<()> {
    ...
    resource_file_contents.push_str("struct AlignedAsset<T>(T);\n\n");
    ...
    for resource in texture_resources {
        ...
        let file_name = resource.file_stem().unwrap().to_ascii_uppercase().into_string().unwrap();
        let byte_size = fs::read(resource_path).unwrap().len();
        resource_file_contents.push_str(format!("const {}: AlignedAsset<[u8; {}]> = AlignedAsset(*include_bytes!(\"../{}\"));\n", file_name, byte_size, resource_path).as_str());
    }
